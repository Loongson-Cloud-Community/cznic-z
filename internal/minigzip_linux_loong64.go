// Code generated by 'ccgo -lmodernc.org/z/lib -o internal/minigzip_linux_loong64.go -trace-translation-units /tmp/go-generate-1786164741/cdb.json minigzip64', DO NOT EDIT.
...
func error(tls *libc.TLS, msg uintptr) { /* minigzip.c:351:6: */
	...
	libc.Xfprintf(tls, libc.Xstderr, ts, libc.VaList(bp, prog, msg))
	libc.Xexit(tls, 1)
}
...
func gz_compress(tls *libc.TLS, in uintptr, out gzFile) { /* minigzip.c:362:6: */
	...
	for {
		len = int32(libc.Xfread(tls, bp, uint64(1), uint64(unsafe.Sizeof([16384]uint8{})), in))
...
func file_compress(tls *libc.TLS, file uintptr, mode uintptr) { /* minigzip.c:457:6: */
	...
	if libc.Xstrlen(tls, file)+libc.Xstrlen(tls, ts+57) >= size_t(unsafe.Sizeof([1024]uint8{})) {
		libc.Xfprintf(tls, libc.Xstderr, ts+61, libc.VaList(bp, prog))
		libc.Xexit(tls, 1)
	}

	libc.Xsnprintf(tls, bp+40, uint64(unsafe.Sizeof([1024]uint8{})), ts+84, libc.VaList(bp+8, file, ts+57))
	...
	out = z.Xgzopen64(tls, bp+40, mode)
	if out == uintptr(0) {
		libc.Xfprintf(tls, libc.Xstderr, ts+92, libc.VaList(bp+24, prog, bp+40))
		libc.Xexit(tls, 1)
	}
...
func file_uncompress(tls *libc.TLS, file uintptr) { /* minigzip.c:496:6: */
	...
	if size_t(len)+libc.Xstrlen(tls, ts+57) >= size_t(unsafe.Sizeof([1024]uint8{})) {
		libc.Xfprintf(tls, libc.Xstderr, ts+61, libc.VaList(bp, prog))
		libc.Xexit(tls, 1)
	}

	libc.Xsnprintf(tls, bp+40, uint64(unsafe.Sizeof([1024]uint8{})), ts+113, libc.VaList(bp+8, file))

	if uint64(len) > uint64(unsafe.Sizeof([4]uint8{}))-uint64(1) && libc.Xstrcmp(tls, file+uintptr(len)-uintptr(uint64(unsafe.Sizeof([4]uint8{}))-uint64(1)), ts+57) == 0 {
		infile = file
		outfile = bp + 40 /* &buf[0] */
		*(*uint8)(unsafe.Pointer(outfile + uintptr(len-uint32(3)))) = uint8(0)
		...
		outfile = file
		infile = bp + 40 /* &buf[0] */
		libc.Xsnprintf(tls, bp+40+uintptr(len), uint64(unsafe.Sizeof([1024]uint8{}))-uint64(len), ts+113, libc.VaList(bp+16, ts+57 /* ".gz" */))
	}
	in = z.Xgzopen64(tls, infile, ts+89)
	if in == uintptr(0) {
		libc.Xfprintf(tls, libc.Xstderr, ts+92, libc.VaList(bp+24, prog, infile))
		libc.Xexit(tls, 1)
	}
...
func main1(tls *libc.TLS, argc int32, argv uintptr) int32 { /* minigzip.c:556:5: */
	...
	libc.Xsnprintf(tls, bp+24, uint64(unsafe.Sizeof([20]uint8{})), ts+113, libc.VaList(bp, ts+119))
	...
	if argc == 0 {
		...
		if uncompr != 0 {
			...
			file = z.Xgzdopen(tls, libc.Xfileno(tls, libc.Xstdout), bp+24)
			if file == uintptr(0) {
				error(tls, ts+171)
			}
		...
		for __ccgo := true; __ccgo; __ccgo = func() bool { argv += 8; return libc.PreDecInt32(&argc, 1) != 0 }() {
			if uncompr != 0 {
				if copyout != 0 {
					file = z.Xgzopen64(tls, *(*uintptr)(unsafe.Pointer(argv)), ts+89)
					if file == uintptr(0) {
						libc.Xfprintf(tls, libc.Xstderr, ts+92, libc.VaList(bp+8, prog, *(*uintptr)(unsafe.Pointer(argv))))
					} else {
						gz_uncompress(tls, file, libc.Xstdout)
					}
				} else {
